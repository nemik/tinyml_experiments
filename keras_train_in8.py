...
def _parse_fn(example_serialized, is_training):
    ...
    image = preprocess_image(image, 96, 96, is_training=is_training)
...
model_ckpt = tf.keras.callbacks.ModelCheckpoint(
        os.path.join("keras_model", "kmod") + '-ckpt-{epoch:03d}.h5',
        monitor='val_loss',
        save_best_only=True)
...
SAVE_DIR = '.'
save_name = "keras_birds"
# training finished
model.save('{}/{}-model-final.h5'.format(SAVE_DIR, save_name))

...
from tensorflow.keras.layers import Input, Dense, GlobalAveragePooling2D, AveragePooling2D
...
# this one will include a Dense layer which for some reason after quantization gets turned into a MEAN
# layer which TFLu does not support
# base_model = MobileNetV2(input_tensor=vww_input, alpha=0.25, classes=2, weights=None, include_top=True)

base_model = MobileNetV2(input_tensor=vww_input, alpha=0.25, weights=None, include_top=False)
...
model_ckpt = tf.keras.callbacks.ModelCheckpoint(
        os.path.join("keras_birds", "mobilenetv2-kmod") + '-ckpt-{epoch:03d}.h5',
        monitor='val_loss',
        save_best_only=True)
...
save_name = "keras_birds_mobilenet_v2"
